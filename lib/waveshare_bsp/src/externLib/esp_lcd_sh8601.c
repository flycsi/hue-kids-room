                                   ...
                                   esp_lcd_panel_handle_t *ret_panel)
{
    ...
    case 16: sh8601->colmod_val = 0x55; fb_bits_per_pixel = 16; break;
    case 18: sh8601->colmod_val = 0x66; fb_bits_per_pixel = 18; break;
    case 24: sh8601->colmod_val = 0x77; fb_bits_per_pixel = 24; break;
    default:
...
static esp_err_t panel_sh8601_reset(esp_lcd_panel_t *panel)
{
    ...
    if (sh8601->reset_gpio_num >= 0) {
        ...
        ESP_RETURN_ON_ERROR(tx_param(sh8601, sh8601->io, LCD_CMD_SWRESET, NULL, 0), TAG, "send SW reset failed");
...
static esp_err_t panel_sh8601_init(esp_lcd_panel_t *panel)
{
    ...
    ESP_RETURN_ON_ERROR(tx_param(sh8601, io, LCD_CMD_MADCTL, (uint8_t[]){sh8601->madctl_val}, 1), TAG, "MADCTL failed");
    ESP_RETURN_ON_ERROR(tx_param(sh8601, io, LCD_CMD_COLMOD, (uint8_t[]){sh8601->colmod_val}, 1), TAG, "COLMOD failed");

    const sh8601_lcd_init_cmd_t *init_cmds = sh8601->init_cmds ? sh8601->init_cmds : vendor_specific_init_default;
    uint16_t init_cmds_size = sh8601->init_cmds ? sh8601->init_cmds_size
                                                 : sizeof(vendor_specific_init_default) / sizeof(sh8601_lcd_init_cmd_t);

    for (int i = 0; i < init_cmds_size; i++) {
        ESP_RETURN_ON_ERROR(tx_param(sh8601, io, init_cmds[i].cmd, init_cmds[i].data, init_cmds[i].data_bytes), TAG, "init cmd failed");
...
static esp_err_t panel_sh8601_draw_bitmap(esp_lcd_panel_t *panel, int x_start, int y_start, int x_end, int y_end, const void *color_data)
{
    ...
    ESP_RETURN_ON_ERROR(tx_param(sh8601, io, LCD_CMD_CASET, (uint8_t[]){
        (x_start >> 8) & 0xFF, x_start & 0xFF, ((x_end-1) >> 8) & 0xFF, (x_end-1) & 0xFF}, 4), TAG, "CASET failed");
    ESP_RETURN_ON_ERROR(tx_param(sh8601, io, LCD_CMD_RASET, (uint8_t[]){
        (y_start >> 8) & 0xFF, y_start & 0xFF, ((y_end-1) >> 8) & 0xFF, (y_end-1) & 0xFF}, 4), TAG, "RASET failed");
...
{
    sh8601_panel_t *sh8601 = __containerof(panel, sh8601_panel_t, base);
    return tx_param(sh8601, sh8601->io, invert_color_data ? LCD_CMD_INVON : LCD_CMD_INVOFF, NULL, 0);
}

static esp_err_t panel_sh8601_mirror(esp_lcd_panel_t *panel, bool mirror_x, bool mirror_y)
{
    sh8601_panel_t *sh8601 = __containerof(panel, sh8601_panel_t, base);
    if (mirror_x) sh8601->madctl_val |= BIT(6); else sh8601->madctl_val &= ~BIT(6);
    if (mirror_y) { ESP_LOGE(TAG, "mirror_y not supported"); return ESP_ERR_NOT_SUPPORTED; }
    return tx_param(sh8601, sh8601->io, LCD_CMD_MADCTL, (uint8_t[]){sh8601->madctl_val}, 1);
}

static esp_err_t panel_sh8601_swap_xy(esp_lcd_panel_t *panel, bool swap_axes)
{
    ESP_LOGE(TAG, "swap_xy not supported"); return ESP_ERR_NOT_SUPPORTED;
}

static esp_err_t panel_sh8601_set_gap(esp_lcd_panel_t *panel, int x_gap, int y_gap)
{
    sh8601_panel_t *sh8601 = __containerof(panel, sh8601_panel_t, base);
    sh8601->x_gap = x_gap; sh8601->y_gap = y_gap;
    return ESP_OK;
}
...
{
    sh8601_panel_t *sh8601 = __containerof(panel, sh8601_panel_t, base);
    return tx_param(sh8601, sh8601->io, on_off ? LCD_CMD_DISPON : LCD_CMD_DISPOFF, NULL, 0);
}
